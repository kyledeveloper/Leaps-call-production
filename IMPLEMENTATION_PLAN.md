...
- **流动性护栏（深实值特化）**：以点差与未平仓量（OI $\ge 300$ Pass, $\ge 100$ Watch, $< 100$ Reject）为主，日均成交量（Volume）不再作为一票否决项。在点差合格且 OI $\ge 100$ 条件下，只要卖一挂单深度 $ask\_size \ge 5$，允许 0 日成交量期权评为 PASS；若 $1 \le ask\_size < 5$ 降级为 WATCH，防范幽灵虚假报价（Defensive Clauses 7-10）。
